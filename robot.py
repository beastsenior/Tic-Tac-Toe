...
class Crobot():
    ...
    def move(self,m,movemode=-1):  #movemode用于指定这一步的模式：默认-1为跟随生成机器人时的指定，0为随机下法，1为DQN训练模式，2为DQN专家模式
        if movemode==-1:
            movemode=self.mode

        #随机模式
        if movemode==0:
            x,y= self.brain_random(m)
            return x,y,self.flag

        #DQN训练模式
        if movemode==1:
            x,y=self.brain_dqn_train.choose_action(m)
            return x,y,self.flag
